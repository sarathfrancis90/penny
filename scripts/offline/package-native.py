...
def configuration(value, platform, store_max):
    ...
    required = common | ({"teamId", "container", "profileUUID", "signingIdentity"} if platform == "ios" else
                         {"driveClientId", "uploadCertificateSha256", "driveSigningSha256", "apksigner", "apkanalyzer"})
    ...
    if platform == "android":
        for name in ("apksigner", "apkanalyzer"):
...
def android_build(config, output, log, workspace=None):
    ...
    outputs = []
    for source in apks + aabs:
        target = output / source.name
        shutil.copyfile(source, target)
        outputs.append(target)
    return outputs, ["android", str(outputs[0]), "--certificate-sha256", config["uploadCertificateSha256"],
