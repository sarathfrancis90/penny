...
@Composable fun PennyApp(vm: PennyViewModel = viewModel()) {
    ...
    state.restorePreview?.let { preview -> AlertDialog(onDismissRequest = vm::cancelRestore, title = { Text("Replace this vault?") },
        text = { Text("This backup contains ${preview.expenses.size} expenses, ${preview.attachments.size} receipts, ${preview.finance.budgets.size} budgets, ${preview.finance.incomeSources.size} income sources, ${preview.finance.incomeEntries.size} received payments, ${preview.finance.savingsGoals.size} savings goals, ${preview.finance.savingsEntries.size} contributions and ${preview.finance.recurringExpenses.size} recurring templates. Expenses total ${Money.format(Money.total(preview.expenses))}. It replaces all current vault data. Create a backup first to keep the current records.") },
