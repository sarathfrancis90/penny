...
Updated 2026-09-15. [PLAN.md](PLAN.md) defines the product and release gates. The [evidence index](evidence/README.md), native READMEs and [REVIEW.md](REVIEW.md) preserve earlier checks, failures and corrections.
...
[Hosted native58726be](evidence/hosted-native-58726be.json) passed every native job: Android API26/API37, Android build, authenticated contract/corpus, iOS simulator and aggregate. iOS records145 unit tests,9 UI tests and two separate generation phases passed; the compact report does not recount Android methods/skips. Retained Flutter iOS and the repository required gate also succeeded. This applies to58726be only; today's later physical test overlays have separate evidence. Historical failed runs below remain unchanged.
...
Earlier [hosted1305c5e](evidence/hosted-native-1305c5e.json) passes both Android runtimes (112 passed, eight named skips and four separate process methods each), native build and authenticated contract/corpus checks. iOS passes143/144 unit methods and9/9 UI methods; one automatic-backup test exhausted a fixed scheduler-yield wait before worker completion. The test-only correction now waits on actual completion under a fixed wall-clock deadline; all six automatic-backup methods pass locally, preserving completion, cancellation and last-good assertions. The failed hosted command did not reach separate iOS generation phases.

1. [Hosted59304d7](evidence/hosted-native-59304d7.json) completed: iOS143/143 unit tests,9/9 UI tests and both separate generation phases passed. Android37 passed112/120 with eight named skips and four separate process methods. Android26 recorded two test-readiness failures; its subsequent process phases did not run. The contract job failed on an HTTP504 source download. Local successor `915a617c80a71837cb2122f7ad9115d38782246a` contains the passing Android test corrections and iOS encoding reuse; the subsequent58726be hosted native run passed as recorded above.
...
The September15 physical session uses Samsung SM-S928U1 (Android16/API36,4KiB pages) and iPhone13 Pro Max (iOS26.6). Android [five integration methods](../../apps/android/evidence/physical-android-2026-09-15.json) passed, followed by a separate [UI journey](../../apps/android/evidence/physical-android-ui-2026-09-15.json) and [two owner-offline methods](../../apps/android/evidence/physical-android-offline-2026-09-15.json). The initial dozing-screen UI failure is retained. Offline OCR asserts no active network and recognizes the synthetic receipt; manual CRUD/recreation/deletion passes with user-owned network settings unchanged. These are distinct runs, not eight unique tests. Nano's UI reports unavailable; unsupported status and caught exceptions share that display, so the underlying SDK reason is not established.

[iOS initial physical QA](../../apps/ios/evidence/physical-ios-2026-09-15.json) passed9/9 with network available, including Vision OCR, local recovery/file protection and UI relaunch. The subsequent [owner-offline selection passed4/4](../../apps/ios/evidence/physical-ios-offline-2026-09-15.json), zero skips, with an asserted `unsatisfied` network path. Vision OCR, exact deterministic receipt fallback, visible model status/manual editor and CRUD across relaunch pass. Foundation Models reports `deviceNotEligible`; this is unavailable-state evidence, not inference quality. Normal QA launch succeeds afterward. The original profile's certificate-check failure recurred offline despite a passing online warmup. Apple's Development Offline profile resolved it using the same certificate/device and unchanged entitlements; separately frozen products preserve both signing variants. The profile expires September22 at18:31:52UTC. CrowdStrike was untouched. Focused device passes do not establish store distribution or the remaining release matrix; [session integration](evidence/physical-session-integration.json) preserves exact scopes and failures.

The [device-session record](DEVICE_SESSION.md) separates these results from outstanding supported-model inference, secure-hardware attestation, physical interruption/resources and real-provider recovery. No personal shipping data was replaced. Android's default-deny TLS configuration does not establish custom SDK/AICore traffic behavior. [PRIVACY.md](PRIVACY.md), [PROVIDER_SETUP.md](PROVIDER_SETUP.md) and [RELEASE.md](RELEASE.md) retain these gates. Store/API observations are historical snapshots, not newly verified release state.
...
The latest recorded account reading is **2% weekly usage consumed / 98% remaining**, resetting **2026-09-22 at 14:01:02 America/Toronto**. This is account-wide, not a repository token budget. Two reset credits remain; none was redeemed by us during this session. Work uses two narrowly scoped implementers plus independent review when a concrete patch is ready, with usage checks at integration milestones.
