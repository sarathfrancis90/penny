...
| Automatic iCloud backup | Provisioned CloudKit private database using immutable `PennyEncryptedObject` records and encrypted CKAssets. Authenticate remote snapshot and manifest readback before recording completion. Local vault remains outside the container. | Signed Apple ID device test, unavailable iCloud account, revoked/changed account, quota, background scheduling, upload/download integrity and second-device recovery. |
...
The implemented Apple transport uses CloudKit, as specified in [CLOUD_BACKUP.md](CLOUD_BACKUP.md), rather than the earlier document-container proposal. It uses explicit snapshot publication and restore; a successful local write is not a verified uploaded backup. The private database must provide the fields and query index documented in the [iOS README](../../apps/ios/README.md).

Continuous record synchronization is not part of this product boundary. iCloud backup is Apple ecosystem-specific; Drive on Android does not automatically restore an iCloud backup. The portable encrypted file plus recovery key provides manual cross-platform transfer. Shared contract tests do not establish real-provider operation; current configuration and device gates are recorded in [PROVIDER_SETUP.md](PROVIDER_SETUP.md).
