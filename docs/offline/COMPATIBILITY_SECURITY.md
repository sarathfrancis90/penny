...
Focused Semgrep 1.165.0 with the CI rulesets passed with zero findings and no parse errors. The shared gate passed 67 Node tests and 22 Python groups, including the AVD profile regression. Full clean-review Semgrep scanning exited successfully with zero findings over 1,072 files, but reported 12 partial-parsing warnings in GitHub expression snippets, shell scripts and retained JSX. This limits scanner coverage; no exclusions were added to hide these warnings. Hosted dependency-security, OSV, aggregate security and SAST passed at `25a776c`; [exact check links](evidence/hosted-security-25a776c.json) record the head. These checks are not a signed native release or complete security assessment. Independent tooling review found no actionable regression and confirmed DTD/entity rejection; local SDK ownership/metadata validation does not authenticate an SDK binary against its publisher.
