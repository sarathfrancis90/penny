...
- [Hosted58726be](hosted-native-58726be.json): all native jobs succeeded; iOS145 units,9 UI methods and two separate generation phases. Android API26/API37/build succeeded; this report does not recount Android methods/skips. Retained Flutter iOS and the repository required gate succeeded. Historical failures and later test overlays remain separate.
- September15 Samsung SM-S928U1/Android16 physical QA: [five integration methods](../../../apps/android/evidence/physical-android-2026-09-15.json), [one UI journey](../../../apps/android/evidence/physical-android-ui-2026-09-15.json), and [two owner-offline methods](../../../apps/android/evidence/physical-android-offline-2026-09-15.json). Initial dozing-screen failure retained; UI recreation is not process death. Actual bundled OCR/no-active-network assertion passes with settings unchanged. Nano unavailable display does not identify the raw SDK reason or establish hardware security level.
- [September15 iPhone13 Pro Max/iOS26.6 initial physical QA](../../../apps/ios/evidence/physical-ios-2026-09-15.json):9/9 with connectivity available; locked-device mount/launch failures retained. The separate [final owner-offline selection](../../../apps/ios/evidence/physical-ios-offline-2026-09-15.json) passes4/4, including asserted absent network path, OCR, actual unavailable-model fallback and manual CRUD across relaunch. Apple's seven-day Development Offline profile resolves the earlier certificate failures. Both signing variants remain frozen; no store-distribution or supported-model inference claim. See [session integration](physical-session-integration.json) and [DEVICE_SESSION.md](../DEVICE_SESSION.md).
