...
final currentUserMembershipProvider =
    FutureProvider.family<GroupMemberModel?, String>((ref, groupId) {
      ...
      return ref
          ...
          .getUserMembership(groupId, user.uid);
    });

/// Stream a single group by ID so detail/header/stats update in real time.
final groupByIdProvider = StreamProvider.family<GroupModel?, String>((
  ref,
  groupId,
) {
  if (ref.watch(guestModeProvider)) return Stream.value(null);
  final user = ref.watch(currentUserProvider);
  if (user == null) return const Stream.empty();
...
final pendingGroupExpensesProvider =
    StreamProvider.family<List<ExpenseModel>, String>((ref, groupId) {
      return ref
          ...
          .watchPendingGroupExpenses(groupId);
    });

/// Count of pending expenses for a group.
final pendingExpenseCountProvider = Provider.family<int, String>((
  ref,
  groupId,
) {
  return ref.watch(pendingGroupExpensesProvider(groupId)).valueOrNull?.length ??
      0;
});
