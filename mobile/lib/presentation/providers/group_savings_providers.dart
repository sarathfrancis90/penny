...
final groupSavingsGoalsProvider =
    StreamProvider.family<List<GroupSavingsGoalModel>, String>((ref, groupId) {
      return ref
...
final groupTotalSavedProvider = Provider.family<double, String>((ref, groupId) {
  final goals = ref.watch(groupSavingsGoalsProvider(groupId)).valueOrNull ?? [];
  return goals.fold(0.0, (sum, g) => sum + g.currentAmount);
});

/// Total target across all group goals.
final groupTotalTargetProvider = Provider.family<double, String>((
  ref,
  groupId,
) {
  final goals = ref.watch(groupSavingsGoalsProvider(groupId)).valueOrNull ?? [];
