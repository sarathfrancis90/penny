...
The app's internal `PennyV4` static target now compiles the existing prototype frame/logical sources directly, with the current app's stateless domain and image validators. `module/PennyV4Reader.swift` exposes an owned bounded source and a read-only summary only; it always runs the native semantic sink. The app adapter imposes the existing expense/receipt caps. No UI, legacy dispatch, backup writer, candidate or installation integration is enabled.

Every Xcode build of this target requires explicit `PENNY_SODIUM_OUTPUT` (the two-slice build directory) and `PENNY_SODIUM_SOURCE` (the verified source tree). Its read-only `verify_build.py` preflight rehashes the source and exact installed input inventory, rejects symlinks and unrecorded headers, verifies the ISC notice, and inspects every archive object's architecture/platform/deployment. It creates no outputs and never fetches or builds a library. The local build report is build provenance, not a signature over a binary supplied by an untrusted third party; authenticated source preparation and the existing controlled builder remain required. Do not modify/reuse the output concurrently with compilation.

`check_build_verification.py --build ABS --source ABS` verifies a real positive build then rejects extra/shadow headers, a header symlink, a changed archive, a same-hash-recorded wrong-platform archive, and wrong deployment on disposable copies. The app bundles the exact root `LICENSE.libsodium`. App-level proof and remaining boundaries are recorded separately in `apps/ios/evidence/v4-app-module.{md,json}`; earlier prototype results are not inherited app evidence.
